		...
		<div class="add-passport_inner mt-4">
			<div class="passport-form_inputs row">
					...
					<div class="col-12">
						...
						<b-form class="d-flex flex-wrap justify-content-between mt-4">
							<b-form-group label="Passport number">
								<form-field :placeholder="'Enter'" v-model="form.customer.name"
							...
							<b-form-group label="Surname">
								<form-field :placeholder="'Enter'" v-model="form.customer.surname"
							...
							<b-form-group label="Patronymic">
								<form-field :placeholder="'Enter'" :is-invalid="$v.form.customer.email.$error"
							...
							<b-form-group label="Nationality">
								<v-select :class="{'is-invalid': $v.form.customer.attrs.citizen_of.$error}" :clearable="true"
									:getOptionLabel="country => country.value" :options="countries" :reduce="data => data.key"
									v-model="form.customer.attrs.citizen_of" @input="fetchVisaTypes">
								</v-select>
								<span class="select-invalid" v-if="$v.form.customer.attrs.citizen_of.$error">
									Input can't be empty
								</span>
							</b-form-group>
							<b-form-group label="Date of issue">
								<date-picker :class="{'invalid': $v.form.visa.requested_date_to.$error}" value-type="format"
									format="YYYY-MM-DD" placeholder="Choose date" v-model="form.visa.requested_date_to" />
								<span class="select-invalid" v-if="$v.form.visa.requested_date_to.$error">
									Input can't be empty
								</span>
							</b-form-group>
							...
							<b-form-group label="Date of issue">
								<date-picker :class="{'invalid': $v.form.visa.requested_date_from.$error}" value-type="format"
									format="YYYY-MM-DD" placeholder="Choose date" v-model="form.visa.requested_date_from" />
								<span class="select-invalid" v-if="$v.form.visa.requested_date_from.$error">
									Input can't be empty
								</span>
							</b-form-group>
				...
				<div class="col-4 d-flex flex-column justify-content-between">
					<client-only>
						<dropzone id="foo" :options="dropzoneOptions" :destroyDropzone="true"></dropzone>
